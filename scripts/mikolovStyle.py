...
def jobman(state, channel):
    ...
    x = TT.lvector('x')
    y = TT.lvector('y')
    h0 = theano.shared(numpy.zeros((eval(state['nhids']),), dtype='float32'))
    ...
    #### Validation
    h0val = theano.shared(numpy.zeros((eval(state['nhids']),), dtype='float32'))
    ...
    def sample_fn(word_tm1, h_tm1):
        ...
        h0 = rec(x_emb, state_before=h_tm1, one_step=True, use_noise=False)
    ...
    [samples, summaries], updates = scan(sample_fn,
                      states = [
                          ...
                          TT.alloc(numpy.float32(0), 1, eval(state['nhids']))],
